...
| **Python 3.10+** | With SciPy, NumPy, OpenCV | `python3 -c "import scipy, cv2"` |
...
| Level | Objectives |
|-------|-----------|
| **🌱 Beginner** | Explain why real-to-sim parameter estimation matters |
...
| **🔧 Intermediate** | Implement camera intrinsic calibration from checkerboard images |
| | Estimate IMU bias and noise parameters using Allan variance |
| | Use optimization to fit simulation parameters to real data |
| **⚡ Advanced** | Design robust parameter estimation pipelines with uncertainty quantification |
...
| **🏗️ Architect** | Design enterprise calibration infrastructure for robot fleets |
...
| **Noise Density** | White noise magnitude per √Hz | Determines short-term precision |
...
- **Forward (Simulation)**: Given parameters → compute outputs
- **Inverse (Real-to-Sim)**: Given outputs → infer parameters
...
│                                                                          │
│  ┌──────────────┐    ┌──────────────┐    ┌──────────────┐               │
│  │   Real       │───▶│   Feature    │───▶│   Parameter  │               │
│  │   Sensor     │    │   Extract    │    │   Estimate   │               │
│  │   Data       │    │              │    │              │               │
│  └──────────────┘    └──────────────┘    └──────────────┘               │
│        │                    │                    │                       │
│        ▼                    ▼                    ▼                       │
│  ┌──────────────┐    ┌──────────────┐    ┌──────────────┐               │
│  │   Camera:    │    │   Corners,   │    │   Intrinsics │               │
│  │   Images     │    │   Points,    │    │   Distortion │               │
│  │   IMU: Raw   │    │   Statistics │    │   Bias/Noise │               │
│  └──────────────┘    └──────────────┘    └──────────────┘               │
│                                                 │                        │
│                            ┌────────────────────┴────────────────────┐  │
│                            ▼                                         ▼  │
...
class SensorParameters:
    ...
    accel_bias: Tuple[float, float, float] = (0.0, 0.0, 0.0)  # m/s²
    ...
    noise_density: Tuple[float, float, float] = (0.01, 0.01, 0.01)  # m/s²/√Hz
...
class PhysicsParameters:
    ...
    )  # kg·m² (3x3 inertia tensor as flat tuple)
    friction_coefficient: float = 0.5  # Coulomb friction μ
...
1. **Image diversity**: Capture the board at various angles (±45°) and distances
...
class IMUParameterEstimator:
    ...
    def estimate_noise_density(
        ...
        the white noise, typically given in units/√Hz.

        Args:
            accel_data: Nx3 accelerometer readings (m/s²)
        ...
        # Convert to noise density (divide by √bandwidth)
        sqrt_dt = np.sqrt(self.dt)

        return {
            'accel_noise_density': accel_noise * sqrt_dt,  # m/s²/√Hz
            'gyro_noise_density': gyro_noise * sqrt_dt,    # rad/s/√Hz
...
- **IMU bias stability**: < 0.01°/h for gyro
...
**Key metrics they care about:**
- **Joint encoder accuracy**: < 0.1° error
...
class SelfCalibratingAgent:
    ...
    def check_calibration_health(self, recent_data: Dict) -> bool:
        ...
        if gravity_error > self.drift_threshold:
            print(f"IMU calibration drift detected: {gravity_error:.3f} m/s²")
            return False
...
4. Verify accelerometer z-axis reads approximately 9.81 m/s²

**Success Criteria:**
- [ ] Collected 6000+ samples at 100 Hz
- [ ] Gyroscope means are close to zero (< 0.01 rad/s)
- [ ] Accelerometer z-mean is within 0.5 m/s² of 9.81

<details>
<summary>💡 Hint</summary>
The gyroscope should read nearly zero on all axes when stationary. Any non-zero mean is the bias. The accelerometer measures gravity, so z-axis should be ~9.81 m/s² for z-up orientation.
</details>
...
print(f"Measured gravity: {accel_mean[2]:.3f} m/s²")
...
### Exercise 2: Camera Calibration (Intermediate) 🔧
...
### Exercise 4: Architect's Design (Expert) 🏗️
...
├── No corners detected
│   ├── Check lighting → Ensure even, bright lighting
│   └── Check focus → Manual focus on calibration distance
├── Corners detected but high error
│   ├── Check board flatness → Use rigid, flat board
│   └── Check image diversity → Cover full frame
└── Optimization fails
    └── Check board dimensions → Verify pattern_size matches

IMU calibration failing?
├── Bias estimates unreasonable
│   ├── Check orientation → Ensure correct z-up assumption
│   └── Check motion → Sensor must be completely stationary
└── High noise estimates
    └── Check sampling rate → Verify actual vs expected rate
```
