...
| **Python 3.10+** | NumPy, SciPy installed | `python3 -c "import numpy, scipy"` |
...
| Level | Objectives |
|-------|------------|
| 🌱 **Beginner** | Define IMU noise types (bias, random walk, scale factor) and explain why filtering is necessary |
| 🔧 **Intermediate** | Implement realistic IMU noise models matching real sensor datasheets (BNO055, BMI088) |
| ⚡ **Advanced** | Tune Madgwick/complementary filters for optimal performance and validate with Allan variance |
| 🏗️ **Architect** | Design sensor fusion architectures for multi-IMU systems with graceful degradation |
...
| **Random Walk** | Accumulated noise from integrating white noise | Gyro integration drift grows with √time |
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│                    IMU DRIFT: THE SILENT KILLER                          │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  True Orientation              IMU Estimate (No Filtering)              │
│                                                                         │
│      ▲                              ▲                                   │
│      │ 0°                           │                                   │
│      │ ───────────────────          │     ╱╱╱╱╱╱╱ ← Drift!              │
...
│      └────────────────▶ Time        └────────────────▶ Time             │
│                                                                         │
│  Robot hasn't moved,              After 10 min: thinks it rotated 2°    │
│  still at 0°                      After 1 hr: thinks it rotated 11°     │
│                                   After 1 day: completely lost          │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

:::tip Why This Matters
Every robot that balances, flies, or navigates relies on IMU orientation estimates. Boston Dynamics' Atlas runs IMU fusion at 1kHz. Drones would crash within seconds without proper filtering. Tesla's Optimus fuses multiple IMUs with visual odometry. Understanding IMU noise isn't optional—it's the foundation of robot state estimation.
:::
...
│     ┌──────────────────────────────┐                                    │
│     │ True: 0.0 rad/s              │                                    │
│     │ Measured: 0.003 rad/s        │ ← Even when perfectly still        │
│     │ Bias = 0.003 rad/s           │                                    │
│     └──────────────────────────────┘                                    │
│                                                                         │
...
│     ┌──────────────────────────────┐                                    │
│     │    ∧    ∧                    │                                    │
│     │   / \  / \   ← White noise   │ Integrates to random walk          │
│     │  /   \/   \                  │ σ grows with √t                    │
│     └──────────────────────────────┘                                    │
│                                                                         │
│  3. BIAS INSTABILITY (Slowly Wandering Bias)                            │
│     ┌──────────────────────────────┐                                    │
│     │      ___/\___                │                                    │
│     │  ___/        \___            │ Bias itself drifts slowly          │
│     │ /                \           │ ~0.01-1 °/hr for MEMS              │
│     └──────────────────────────────┘                                    │
│                                                                         │
...
│     ┌──────────────────────────────┐                                    │
│     │ True rotation: 90°           │                                    │
│     │ Measured: 89.5° or 90.5°     │ ← 0.5% scale error                 │
│     │ Error grows with motion      │                                    │
│     └──────────────────────────────┘                                    │
│                                                                         │
│  5. AXIS MISALIGNMENT                                                   │
│     ┌──────────────────────────────┐                                    │
│     │      Y (measured)            │                                    │
│     │       ↗  ← 0.5° off          │ Physical axes not perfectly        │
│     │      ↑ Y (true)              │ orthogonal or aligned              │
│     │      │                       │                                    │
│     └──────────────────────────────┘                                    │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
...
**Allan variance** is the gold standard for characterizing IMU noise. It analyzes how noise behaves at different averaging times (τ).
...
│    │    \  (slope = -1/2)                                               │
│    │     \                                                              │
│    │      \_____  Bias Instability (minimum)                            │
│    │            \_____                                                  │
│    │                  \  Rate Random Walk                               │
│    │                   \  (slope = +1/2)                                │
│    │                                                                    │
...
│                                                                         │
│  Key Parameters:                                                        │
│  • ARW (Angle Random Walk): Noise at τ=1s, units: °/√hr                │
│  • Bias Instability: Minimum of curve, units: °/hr                     │
│  • RRW (Rate Random Walk): Long-term drift, units: °/hr/√hr            │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

:::info 🏗️ Architect's View: IMU Selection Guide
...
|-------------|-----------------|-----|-------------|------|
| Consumer drone | 10-50 °/hr | 0.3 °/√hr | MPU6050 | $5 |
| Industrial robot | 1-10 °/hr | 0.1 °/√hr | BMI088 | $15 |
| Autonomous vehicle | 0.1-1 °/hr | 0.02 °/√hr | ADIS16470 | $200 |
| Navigation grade | &lt;0.01 °/hr | &lt;0.005 °/√hr | HG1700 | $5,000+ |

**Design Decision**: For humanoid robots requiring 1-hour operation without GPS, you need bias instability &lt;1°/hr or visual-inertial fusion.
:::
...
class IMUSpec:
    ...
    # Accelerometer
    accel_noise_density: float      # µg/√Hz
    accel_bias_instability: float   # µg
    accel_range: float              # g

    # Gyroscope
    gyro_noise_density: float       # °/s/√Hz
    gyro_bias_instability: float    # °/hr
    gyro_range: float               # °/s
...
IMU_SPECS = {
    'bno055': IMUSpec(
        name='Bosch BNO055',
        accel_noise_density=150,      # µg/√Hz
        accel_bias_instability=40,    # µg
        accel_range=16.0,             # g
        gyro_noise_density=0.014,     # °/s/√Hz
        gyro_bias_instability=3.0,    # °/hr
        gyro_range=2000.0             # °/s
    ),
    'bmi088': IMUSpec(
        name='Bosch BMI088',
...
class IMUNoiseSimulator:
    ...
    def __init__(self, model: str = 'bno055', seed: Optional[int] = None):
        ...
        self.gyro_bias_instability = np.deg2rad(self.spec.gyro_bias_instability / 3600)  # °/hr → rad/s
        ...
        # Axis misalignment (typically 0.1-0.5°)
        ...
        print(f"  Accel noise: {self.spec.accel_noise_density} µg/√Hz")
        print(f"  Gyro noise: {self.spec.gyro_noise_density} °/s/√Hz")
        print(f"  Gyro bias instability: {self.spec.gyro_bias_instability} °/hr")
    ...
    def add_accel_noise(self, accel: np.ndarray, dt: float) -> np.ndarray:
        ...
        Args:
            accel: True acceleration [ax, ay, az] in m/s²
    ...
    def add_gyro_noise(self, gyro: np.ndarray, dt: float) -> np.ndarray:
        ...
        self._gyro_bias = np.clip(self._gyro_bias, -0.01, 0.01)  # ~0.5°/s max
    ...
    def simulate(self, accel_true: np.ndarray, gyro_true: np.ndarray,
        ...
        Args:
            accel_true: True acceleration [ax, ay, az] m/s²
...
- Accelerometer noise: µg/√Hz, mg/√Hz, or m/s²/√Hz
- Gyroscope noise: °/s/√Hz, °/hr/√Hz, or rad/s/√Hz
- Bias instability: °/hr (gyro) vs µg (accel)

Always convert to SI units (m/s², rad/s) internally and double-check your conversions!
:::
...
class MadgwickFilter:
    ...
    def update(self, accel: np.ndarray, gyro: np.ndarray,
        ...
        Args:
            accel: Accelerometer reading [ax, ay, az] in m/s² or g
...
class ComplementaryFilter:
    ...
    def update(self, accel: np.ndarray, gyro: np.ndarray,
        ...
        # Only valid when acceleration ≈ gravity
...
def plot_results(results: dict, save_path: str = None):
    ...
    axes[2].plot(t, results['integrated'][:, 2], 'r-', alpha=0.5,
                 label=f'Drift: {results["integrated"][-1, 2]:.1f}°')
...
if __name__ == '__main__':
    ...
    print(f"True state: Roll=0°, Pitch=0°, Yaw=0°")
    print(f"Pure integration: Roll={results['integrated'][-1, 0]:.2f}°, "
          f"Pitch={results['integrated'][-1, 1]:.2f}°, "
          f"Yaw={results['integrated'][-1, 2]:.2f}°")
    print(f"Madgwick filter:  Roll={results['madgwick'][-1, 0]:.2f}°, "
          f"Pitch={results['madgwick'][-1, 1]:.2f}°, "
          f"Yaw={results['madgwick'][-1, 2]:.2f}°")
    print(f"Complementary:    Roll={results['complementary'][-1, 0]:.2f}°, "
          f"Pitch={results['complementary'][-1, 1]:.2f}°, "
          f"Yaw={results['complementary'][-1, 2]:.2f}°")

    # Demonstrate drift problem
    print(f"\n⚠️  Without filtering, yaw drifted {results['integrated'][-1, 2]:.1f}° "
...
[IMU Simulator] Initialized Bosch BNO055
  Accel noise: 150 µg/√Hz
  Gyro noise: 0.014 °/s/√Hz
  Gyro bias instability: 3.0 °/hr
Running 60.0s simulation at 100.0Hz...

=== Results After 60 Seconds ===
True state: Roll=0°, Pitch=0°, Yaw=0°
Pure integration: Roll=0.34°, Pitch=-0.28°, Yaw=2.87°
Madgwick filter:  Roll=0.02°, Pitch=0.01°, Yaw=1.43°
Complementary:    Roll=0.01°, Pitch=-0.02°, Yaw=2.15°

⚠️  Without filtering, yaw drifted 2.87° in just 60 seconds!
```

---

:::info 🏭 Industry Spotlight: Tesla Optimus
...
:::info 🏭 Industry Spotlight: Boston Dynamics Atlas

**How Boston Dynamics handles extreme dynamics:**
Atlas experiences accelerations up to 10g during jumping and tumbling—far exceeding typical IMU specifications.

**Their approach:**
1. **Sensor Selection**: High-g accelerometers (±40g range)
...
2. Detect when robot is stationary (acceleration ≈ gravity, gyro ≈ zero)
...
├── Yes: Is it drifting over time?
│   ├── Yes: Which axis?
│   │   ├── Yaw only → Normal without magnetometer
│   │   └── Roll/Pitch too → Check accelerometer calibration
│   └── No: Constant offset?
│       ├── Yes → Calibrate sensor biases
│       └── No: Oscillating?
│           └── Reduce filter gain (beta/alpha)
└── No: Proceed - filter working correctly
```
