...
| **Python 3.11+** | asyncio, threading | `python3 -c "import asyncio"` |
...
| Level | Objectives |
|-------|------------|
| 🌱 **Beginner** | Define pipeline latency components and explain why latency matters for robotics |
| 🔧 **Intermediate** | Implement latency simulation with configurable delays and statistical distributions |
| ⚡ **Advanced** | Validate real-time performance, identify bottlenecks, and optimize pipeline timing |
| 🏗️ **Architect** | Design latency-aware perception systems with buffering, prediction, and graceful degradation |
...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  Ball Position at t=0ms              Ball Position at t=120ms           │
│  (when camera captures)              (when robot reacts)                │
│                                                                         │
│       ●───────────────────────────────────────────────▶ ●               │
│    (here)                  1.2 meters                 (here!)           │
│                                                                         │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │                    LATENCY BREAKDOWN                             │   │
│  ├─────────────────────────────────────────────────────────────────┤   │
│  │  Capture  │ Transfer │ Detect  │  Plan   │ Execute │  TOTAL    │   │
│  │   10ms    │   5ms    │  50ms   │  30ms   │  25ms   │  120ms    │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                                                                         │
│  At 10 m/s ball speed: 120ms latency = 1.2m prediction error!          │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│                    SENSOR PIPELINE LATENCY BREAKDOWN                     │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  ┌─────────┐   ┌─────────┐   ┌─────────┐   ┌─────────┐   ┌─────────┐  │
│  │ CAPTURE │──▶│TRANSFER │──▶│ PROCESS │──▶│ DECIDE  │──▶│ EXECUTE │  │
│  └─────────┘   └─────────┘   └─────────┘   └─────────┘   └─────────┘  │
│                                                                         │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │ Stage        │ Typical │ Source of Delay                        │   │
│  ├──────────────┼─────────┼────────────────────────────────────────┤   │
│  │ Capture      │ 5-33ms  │ Sensor exposure, readout, triggering   │   │
│  │ Transfer     │ 1-20ms  │ USB, Ethernet, PCIe bandwidth          │   │
│  │ Preprocess   │ 2-10ms  │ Debayer, undistort, resize             │   │
│  │ Inference    │ 10-100ms│ Neural network, feature extraction     │   │
│  │ Postprocess  │ 1-5ms   │ NMS, tracking, filtering               │   │
│  │ Planning     │ 5-50ms  │ Path planning, collision checking      │   │
│  │ Control      │ 1-5ms   │ Trajectory generation, IK              │   │
│  │ Actuation    │ 5-20ms  │ Motor commands, mechanical response    │   │
│  └─────────────────────────────────────────────────────────────────┘   │
│                                                                         │
│  TOTAL: 30-250ms typical for vision-based manipulation                 │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  Frame Request    Exposure Start    Exposure End      Data Ready        │
│       │                │                 │                │             │
│       ▼                ▼                 ▼                ▼             │
│  ─────●────────────────●─────────────────●────────────────●─────▶ t    │
│       │                │                 │                │             │
...
│       │    Delay       │    Time         │    Time        │             │
│       │   (0-1ms)      │  (0.1-33ms)     │   (1-10ms)     │             │
│                                                                         │
│  Total Capture Latency = Trigger + Exposure/2 + Readout                 │
│                                                                         │
│  Examples:                                                              │
│  • Global shutter @ 30fps: ~17ms (half exposure + readout)              │
│  • Rolling shutter: varies by row (adds motion artifacts)               │
│  • High-speed camera @ 120fps: ~4ms                                     │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
```python
# Typical processing times (on RTX 5080/6080)
...
:::tip 💡 Elite Insight: Latency Compensation Strategies
...
:::info 🏭 Industry Spotlight: Tesla Autopilot/FSD
...
**Lesson learned**: At highway speeds, 100ms latency means planning for where objects *will be*, not where they *are*. Prediction is not optional—it's essential.
:::

---

:::info 🏭 Industry Spotlight: Amazon Robotics (Kiva/Proteus)
...
- Stale world model → poor decisions
- Missed events → reactive instead of proactive
- Uncertainty growth → wider safety margins
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│                         CAPSTONE LATENCY BUDGET                          │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│    Voice ───▶ Plan ───▶ Navigate ───▶ Vision ───▶ Manipulate           │
│    50ms       100ms      30ms          50ms        70ms                 │
│                                                                         │
│    Total Budget: 300ms (for responsive human interaction)               │
│                                                                         │
│    ┌─────────────────────────────────────────────────────────┐          │
│    │              LATENCY SIMULATION                         │          │
│    │           (This Section - M2-C2-S6)                     │          │
│    │                                                         │          │
│    │  • Model each component's latency distribution          │          │
│    │  • Identify bottlenecks in pipeline                     │          │
│    │  • Validate real-time constraints in simulation         │          │
│    │  • Test latency compensation algorithms                 │          │
│    └─────────────────────────────────────────────────────────┘          │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
...
| **Jitter** | Latency variation—often more problematic than mean latency |
| **Data Age** | Time since capture—determines relevance of perception |
...
| **Bottleneck** | Highest-latency component—focus optimization here |
...
4. Change inference mean from 25ms to 50ms—what happens to p95?
...
The p95 latency represents the worst 5% of cases—important for real-time guarantees. When you increase one component, the total increases, but the distribution also changes.
</details>
...
│   ├── Consistent high → Pipeline bottleneck
│   │   └── Profile each stage to find slowest
│   └── Spiky → System interference
│       ├── Check CPU load: top, htop
│       ├── Check memory: free -h
│       └── Check I/O: iotop
├── Is it only at startup?
│   ├── Yes → JIT compilation, cache warming
│   └── No → Continuous issue
└── Does it correlate with data size?
    ├── Yes → Transfer or processing bottleneck
    └── No → Fixed overhead (capture, setup)
```
