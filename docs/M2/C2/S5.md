...
| **Python 3.11+** | NumPy, OpenCV, SciPy | `python3 -c "import numpy, cv2, scipy"` |
| **Linear Algebra** | Matrix transforms, homogeneous coordinates | 3D coordinate transformations |
| **ROS 2 Kilted Kaiju** | sensor_msgs, PointCloud2 | `ros2 interface show sensor_msgs/msg/PointCloud2` |
...
| Level | Objectives |
|-------|------------|
| 🌱 **Beginner** | Explain how RGB-D cameras work and why alignment is necessary for colored point clouds |
| 🔧 **Intermediate** | Implement depth-to-point-cloud conversion with camera intrinsics and RGB-D alignment |
| ⚡ **Advanced** | Apply voxel grid filtering, statistical outlier removal, and RANSAC plane segmentation |
| 🏗️ **Architect** | Design real-time point cloud processing pipelines for manipulation and navigation |
...
Your humanoid robot's cameras see a table with a red apple. The RGB camera shows the apple's color and texture. The depth camera shows distance—but which pixel corresponds to which 3D point? And how do we merge these to know "there's a red object 0.5m away at position (x, y, z)"?

This is **RGB-D alignment**—the process of combining color and depth information into a unified 3D representation called a **point cloud**. Every point has both position (x, y, z) and color (r, g, b), enabling the robot to recognize objects in 3D space.
...
│                                                                         │
│  ┌─────────────┐     ┌─────────────┐     ┌─────────────────────────┐   │
│  │  RGB Image  │     │ Depth Image │     │   Colored Point Cloud   │   │
│  │             │     │             │     │                         │   │
│  │  ┌───────┐  │     │  ░░▓▓██░░  │     │      • • •              │   │
│  │  │🍎 Red │  │  +  │  ░▓▓▓▓██░  │ ──▶ │    • 🔴 • •            │   │
│  │  │ Apple │  │     │  ░░▓▓██░░  │     │      • • •              │   │
...
│  └─────────────┘     └─────────────┘     └─────────────────────────┘   │
│                                                                         │
│  Color: What it looks like    Depth: How far    Combined: 3D + Color   │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
RGB and depth cameras are physically separate—typically 25-50mm apart. They have different:
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│                    RGB-D SENSOR GEOMETRY                                 │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  Side View:                                                             │
│                                                                         │
│       RGB Camera          Depth Camera (IR)                             │
│          ╔═╗                  ╔═╗                                       │
│          ║ ║←──── 50mm ────→║ ║                                        │
│          ╚═╝                  ╚═╝                                       │
│           │\                  │\                                        │
...
│      RGB FOV   │   Depth FOV                                            │
│                │                                                        │
│       Different pixels see the same 3D point!                           │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
**Without Alignment:**
- Color doesn't match depth—apple appears at wrong location
...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  1. Depth Pixel (u_d, v_d)                                              │
│     │                                                                   │
│     ▼ Backproject with depth intrinsics                                 │
│  2. 3D Point in Depth Frame (X_d, Y_d, Z_d)                             │
│     │                                                                   │
│     ▼ Transform with extrinsics (R, T)                                  │
│  3. 3D Point in RGB Frame (X_c, Y_c, Z_c)                               │
│     │                                                                   │
│     ▼ Project with RGB intrinsics                                       │
│  4. RGB Pixel (u_c, v_c)                                                │
│     │                                                                   │
│     ▼ Sample color                                                      │
│  5. Colored 3D Point (X, Y, Z, R, G, B)                                 │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

:::info 🏗️ Architect's View: Alignment Approaches
...
:::tip 💡 Elite Insight: Voxel Grid Performance
    ...
    def _process_frame(self):
        ...
        if self.frame_count % 30 == 0:
            ...
            self.get_logger().info(
                f'Frame {self.frame_count}: {initial_count}→{after_voxel}→{after_sor} points, '
...
4. **Grasp Planning**: Point cloud → grasp pose in &lt;100ms
...
**Lesson learned**: The biggest challenge isn't algorithms—it's handling the massive variety of items (size, shape, reflectivity, transparency). Domain randomization in simulation is critical.
:::

---

:::info 🏭 Industry Spotlight: Surgical Robotics (Intuitive Surgical)
...
- Detect horizontal planes (±10° from horizontal)
...
├── Empty cloud?
│   ├── Check depth topic: ros2 topic hz /camera/depth/image_rect_raw
│   └── Verify depth values: ros2 topic echo /camera/depth/... --field data
├── Colors wrong?
│   ├── Check RGB-D sync: Compare timestamps
│   └── Verify extrinsics: Print R, T matrices
├── Too sparse?
│   ├── Reduce voxel_size parameter
│   └── Check max_depth setting
└── Too noisy?
    ├── Increase statistical outlier removal
    └── Add radius outlier removal
```
...
### Tutorials
- [ROS 2 Point Cloud Tutorial](https://docs.ros.org/en/kilted/Tutorials/Intermediate/Tf2/Writing-A-Tf2-Listener.html)
