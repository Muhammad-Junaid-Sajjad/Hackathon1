...
| **Python 3.11+** | With OpenCV, NumPy installed | `python3 -c "import cv2, numpy"` |
...
| **🏗️ Architect** | Design enterprise synthetic data infrastructure |
...
**The Reality Gap**: Models trained only on synthetic data often fail on real sensors. Domain randomization is like "vaccination"—exposing the model to many variations so it becomes robust.
:::
...
│  ┌──────────────┐    ┌──────────────┐    ┌──────────────┐               │
│  │   Scene      │───▶│   Domain     │───▶│   Sensor     │               │
│  │   Generator  │    │  Randomizer  │    │  Simulator   │               │
│  └──────────────┘    └──────────────┘    └──────────────┘               │
│        │                    │                    │                       │
│        ▼                    ▼                    ▼                       │
│  ┌──────────────┐    ┌──────────────┐    ┌──────────────┐               │
│  │   Object     │    │   Lighting   │    │   RGB-D      │               │
│  │   Spawning   │    │   Textures   │    │   Images     │               │
│  │   Positions  │    │   Cameras    │    │   Labels     │               │
│  └──────────────┘    └──────────────┘    └──────────────┘               │
│                                                 │                        │
│                            ┌────────────────────┴────────────────────┐  │
│                            ▼                                         ▼  │
...
class DatasetAnalyzer:
    ...
    def print_report(self):
        ...
        print(f"  Small (<32²): {dist['small_boxes']}")
        print(f"  Medium (32²-96²): {dist['medium_boxes']}")
        print(f"  Large (>96²): {dist['large_boxes']}")

        print("\nImage Statistics:")
        img_stats = report["image_statistics"]
        print(f"  Brightness: {img_stats['brightness_mean']:.1f} ± {img_stats['brightness_std']:.1f}")
        print(f"  Contrast: {img_stats['contrast_mean']:.1f} ± {img_stats['contrast_std']:.1f}")

        print("\nDepth Statistics:")
        depth_stats = report["depth_statistics"]
        print(f"  Valid pixels: {depth_stats['valid_pixel_ratio_mean']*100:.1f}%")
        print(f"  Depth range: {depth_stats['depth_mean']:.0f} ± {depth_stats['depth_std']:.0f}")
...
Tesla's Autopilot and FSD teams use simulation to generate edge cases that are rare in real driving data—pedestrians in unusual poses, extreme weather, construction zones. Their "shadow mode" validates simulation accuracy by comparing synthetic predictions against real-world outcomes.
...
**Lessons learned:**
"Simulate the long tail. The common cases are easy—focus synthetic data on rare configurations that cause real failures."
:::
...
- False negatives → missed objects → collisions
- False positives → phantom obstacles → inefficient paths
- Depth errors → grasp failures
...
### Exercise 2: Domain Randomization (Intermediate) 🔧
...
**Bonus Challenges:**
- [ ] Add curriculum learning (easy→hard progression)
...
```
Images all similar?
├── Yes: Check randomization config
│   ├── Config has variation → Check if random seed is fixed
│   └── Config has no variation → Increase randomization ranges
└── No: Proceed to label check

Bounding boxes incorrect?
├── Yes: Check intrinsic matrix
│   ├── Intrinsics correct → Check object positions
│   └── Intrinsics wrong → Recalibrate camera
└── No: Proceed to export check
```
...
│                                                              │
│  ┌──────────┐    ┌──────────┐    ┌──────────┐              │
│  │ Gazebo   │───▶│ Domain   │───▶│ Export   │              │
│  │ Renderer │    │ Random.  │    │ Pipeline │              │
│  └──────────┘    └──────────┘    └──────────┘              │
│       │                │               │                    │
│       ▼                ▼               ▼                    │
│  ┌──────────┐    ┌──────────┐    ┌──────────┐              │
│  │ Ground   │    │ Quality  │    │ Training │              │
│  │ Truth    │    │ Metrics  │    │ Pipeline │              │
│  └──────────┘    └──────────┘    └──────────┘              │
│                                        │                    │
│                                        ▼                    │
│                                  ┌──────────┐              │
