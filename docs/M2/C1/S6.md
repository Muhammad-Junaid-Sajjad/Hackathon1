...
**Joint controllers** translate high-level commands (target positions, velocities, efforts) into low-level actuation forces that move robot joints in simulation. Gazebo Harmonic integrates with **ros2_control** to provide industry-standard controllers: **PID position control**, **velocity control**, **effort (torque) control**, **impedance control**, and **trajectory control**. Accurate joint control is essential for realistic manipulation, locomotion, and contact-rich tasks.
...
- **ros2_control installed**: `sudo apt install ros-humble-ros2-control ros-humble-ros2-controllers`
...
class PIDTuner(Node):
    ...
    def analyze_response(self):
        ...
        # Tuning suggestions
        if overshoot > 10:
            print('⚠️  High overshoot → Reduce Kp or increase Kd')
        if rise_time > 1.0:
            print('⚠️  Slow rise → Increase Kp')
        if settling_time > 2.0:
            print('⚠️  Slow settling → Increase Kd')
        if abs(final_value - self.target_position) > 0.05:
            print('⚠️  Steady-state error → Increase Ki')
        ...
        print('📊 Plot saved to /tmp/pid_step_response.png')
    ...
    def joint_callback(self, msg):
        ...
        if 'gripper_joint' in msg.name:
            ...
            if abs(velocity) < 0.01 and not self.gripper_closed:
                self.get_logger().info('✓ Contact detected, applying grasp force')
...
├─────────────────────────────────────────────────────────┤
│  Planner (High-level)  →  Goal: "Pick up cup"          │
│         ↓                                               │
│  Motion Planner        →  Trajectory: [q0, q1, q2...]  │
│         ↓                                               │
│  Controller (Low-level) →  Execute with compliance     │
│         ↓                                               │
│  Feedback              →  Contact detected, adapt      │
└─────────────────────────────────────────────────────────┘
```
