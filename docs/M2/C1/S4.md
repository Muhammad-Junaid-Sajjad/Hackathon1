...
- **ROS 2 Humble workspace**: With Gazebo Harmonic integration
- **Python 3.10+**: With numpy and scipy for grasp analysis

**Hardware Requirements**:
- 8-core CPU for real-time contact simulation
- GPU recommended for contact-rich scenarios (100+ contact points)
...
|------|------------|----------------|
| **Friction Coefficient (μ)** | Ratio of friction force to normal force | Determines slip resistance |
| **Contact Stiffness (kp)** | Spring constant for contact forces (N/m) | Controls surface hardness |
| **Contact Damping (kd)** | Damping coefficient (N·s/m) | Prevents bouncing |
| **Normal Force** | Force perpendicular to contact surface | Primary grasping force |
| **Tangential Force** | Force parallel to contact surface | Causes slip if exceeds μ×N |
| **Force Closure** | Condition where contacts can resist any external wrench | Grasp stability criterion |
| **Friction Cone** | Region of valid friction forces (angle = arctan(μ)) | Geometric grasp analysis |
...
- **Grip force**: ±5% accuracy vs. real
- **Reliability**: 99.99% grasp success in sim → 99.9% in real
...
**Critical Parameters**:
- **Friction (μ)**: Gripper rubber: 1.5, Steel: 0.4, Ice: 0.05
- **Stiffness (kp)**: Steel: 1e8 N/m, Rubber: 1e5 N/m
- **Damping (kd)**: Prevents bouncing, typical: 1.0-10.0 N·s/m
...
| **kd** | Contact damping (N·s/m) | 0.1-10.0 (higher = less bounce) |
        ...
        if total_normal_force > self.grasp_force_threshold:
            ...
            if total_normal_force * mu > total_tangential_force:
                self.get_logger().info(
                    f'✓ Stable grasp: Normal={total_normal_force:.2f}N, '
                    f'Tangential={total_tangential_force:.2f}N'
                )
            else:
                self.get_logger().warn(
                    f'⚠ Unstable grasp: slip likely! '
...
1. **Timestep**: Use ≤1ms for contact-rich tasks (see M2-C1-S2)
...
```
┌─────────────────────────────────────────────────────────┐
│                 AI Manipulation Agent                    │
├─────────────────────────────────────────────────────────┤
│  1. Perceive object → Estimate grasp candidates         │
│  2. For each candidate:                                  │
│     - Predict contact points                            │
│     - Compute expected forces                           │
│     - Check force closure                               │
│  3. Select best grasp → Execute                         │
│  4. Monitor contact forces → Detect slip/failure        │
│  5. Adapt grip force in real-time                       │
└─────────────────────────────────────────────────────────┘
```
...
The trend is toward "imagination-based grasping"—agents that simulate contacts mentally before physical execution.
:::
...
4. Verify friction using: μ = tan(θ) at slip angle

**Success Criteria:**
- [ ] Box stays stationary on plane at θ &lt; arctan(μ)
- [ ] Box slides when θ &gt; arctan(μ)
- [ ] Verified for μ = 0.3, 0.6, 1.0

<details>
<summary>💡 Hint</summary>
For μ = 0.6, the critical angle is arctan(0.6) ≈ 31°. Tilt the plane to 30° (stable) and 35° (sliding).
</details>
...
| No friction (sliding) | μ = 0 | Set mu and mu2 &gt; 0 |
...
Grasp failing?
├── Object slipping?
│   ├── Check friction: Is μ > 0.5?
│   │   ├── Yes → Check normal force (increase grip force)
│   │   └── No → Increase friction coefficient
│   └── Check tangential force / (μ × normal)
│       ├── Ratio > 1 → Unstable, reposition gripper
│       └── Ratio < 0.8 → Stable, other issue
├── Object vibrating?
│   ├── Reduce contact stiffness (kp)
│   └── Increase damping (kd)
└── No contact detected?
...
| Material | μ | kp (N/m) | kd (N·s/m) | Restitution |
|----------|---|----------|------------|-------------|
