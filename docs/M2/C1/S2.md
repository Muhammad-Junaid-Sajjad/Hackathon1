...
- **ROS 2 Kilted Kaiju workspace**: Sourced and functional (`ros2 topic list` works)
...
- **Python 3.11+**: For running benchmark scripts

**Hardware Requirements**:
- 16-core CPU minimum for 2025 multi-threaded physics
- 32GB RAM recommended for complex humanoid scenes
- NVIDIA RTX 5080/6080 (16GB+ VRAM) for Bullet GPU acceleration (2025 Baseline)
...
- Ubuntu 24.04 LTS
- NVIDIA RTX 5080/6080 (16GB+ VRAM) for 2025 GPU physics (Bullet/Isaac)
...
**Lessons learned:**
Physics accuracy matters less than physics *consistency* for RL—models can adapt to systematic errors but not random ones.
:::
...
**Lessons learned:**
For fleet simulation, GPU-accelerated physics pays off at 10+ robots. Below that, the CPU→GPU transfer overhead dominates.
:::

:::info Industry Spotlight: Intuitive Surgical (da Vinci)
**How Intuitive Surgical uses physics simulation:**
Surgical robot simulation requires extremely high contact fidelity—millimeter-scale tissue interactions with force feedback. They use DART-like custom engines with 0.2ms timesteps.
...
**Lessons learned:**
Medical robotics prioritizes accuracy over speed—a 2x slower but more accurate simulation is preferred over fast but approximate physics.
:::
...
```python
# Rule of Thumb: Timestep ≤ 1 / (10 * highest_frequency_in_system)
...
```
┌─────────────┐     ┌──────────────┐     ┌─────────────┐
│   Sensors   │────▶│  AI Planner  │────▶│   Physics   │
│ (Gazebo)    │     │  (LLM/RL)    │     │   Engine    │
└─────────────┘     └──────────────┘     └─────────────┘
       ▲                   │                    │
       └───────────────────┴────────────────────┘
                    Feedback Loop
...
The trend is toward "physics-informed" AI—models that understand Newton's laws implicitly through massive simulation exposure.
:::
...
RTF too low (<1.0)?
├── Yes: Check CPU usage
│   ├── CPU maxed → Reduce solver iterations or increase timestep
│   └── CPU not maxed → Check for I/O bottleneck (logging?)
└── No: Performance acceptable

Objects behaving strangely?
├── Falling through surfaces → Reduce timestep
├── Jittering/vibrating → Increase solver iterations
├── Exploding apart → Check for interpenetrating initial poses
└── Sliding unexpectedly → Increase friction coefficient
```
