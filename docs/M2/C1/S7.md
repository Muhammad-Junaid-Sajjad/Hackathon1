...
- **ros_gz installed**: `sudo apt install ros-humble-ros-gz`
- **Gazebo Harmonic running**: World with sensors and robot
...
|------|------------|----------------|
| **ros_gz_bridge** | Node that converts Gazebo ↔ ROS 2 messages | Core integration component |
...
| **Message Conversion** | Type translation (gz.msgs → sensor_msgs) | Enables interoperability |
...
/world/default/model/     ←→ Message Translation ←→  /joint_states
  /joint_state                                        /cmd_vel
...
# 7. Velocity Commands (ROS 2 → Gazebo)
...
**Direction Syntax**:
- `[` = Gazebo → ROS 2 (GZ_TO_ROS)
- `]` = ROS 2 → Gazebo (ROS_TO_GZ)
- `@` = Bidirectional
...
class IntegrationTest(Node):
    ...
    def test_bridge(self):
        ...
        # Test ROS 2 → Gazebo
        print('\nTesting ROS 2 → Gazebo...')
        cmd = Twist()
        cmd.linear.x = 0.1
        self.cmd_pub.publish(cmd)
        print('✓ Sent velocity command to Gazebo')
...
│                    AI Agent (ROS 2)                      │
├─────────────────────────────────────────────────────────┤
│  Perception  ←─ ros_gz_bridge ←─  Gazebo Sensors        │
│  Planning    ─→ ros_gz_bridge ─→  Gazebo Actuators      │
│  Feedback    ←─ ros_gz_bridge ←─  Gazebo State          │
└─────────────────────────────────────────────────────────┘
```
...
|-----------|--------|---------|
| Gazebo → ROS 2 | `[` | `/camera@Image[gz.msgs.Image` |
| ROS 2 → Gazebo | `]` | `/cmd_vel@Twist]gz.msgs.Twist` |
...
**Module 2 Chapter 1 Complete!** ✅

You have mastered:
- Gazebo Harmonic installation and ROS 2 integration (S1)
...
- **Image not displaying**: Install `ros-humble-rqt-image-view`, check encoding (RGB8 vs BGR8)
