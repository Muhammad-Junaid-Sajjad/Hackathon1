---
id: m2-c1-s1
title: Gazebo Ionic Installation and ROS 2 Integration
sidebar_position: 1
keywords: ['gazebo', 'ionic', 'installation', 'ros2-gz', 'simulation', 'physics', 'digital-twin', 'sim-to-real']
---

# Gazebo Ionic Installation and ROS 2 Integration
...
|-------------|-------------|--------------|
| **M1-C1-S1** | Ubuntu 24.04 + NVIDIA drivers | `nvidia-smi` shows GPU |
| **M1-C1-S4** | ROS 2 Kilted Kaiju installed | `ros2 --version` returns 2025.x.x |
| **M1-C3-S1** | Humanoid URDF created | File exists in `~/ros2_ws/src/humanoid_description/urdf/` |
| **Hardware** | RTX 5080/6080 (16GB+ VRAM) | Required for 2025 GPU physics & rendering |
...
| Level | Objective |
|-------|-----------|
| 🌱 **Beginner** | Define what a physics simulator does and why we need digital twins |
| 🌱 **Beginner** | Install Gazebo Ionic and verify it runs correctly |
| 🔧 **Intermediate** | Configure the ros_gz_bridge to connect Gazebo and ROS 2 topics |
| 🔧 **Intermediate** | Spawn URDF robots into Gazebo simulation environments |
| ⚡ **Advanced** | Add sensor plugins (IMU, camera) to URDFs for simulation |
| 🏗️ **Architect** | Design simulation architectures for multi-robot AI training systems |
...
**If you've used ROS 2 but not Gazebo Ionic**, focus on:
...
Now imagine you could test 10,000 falls, 100,000 grasps, and 1 million collisions—all before the physical robot takes its first step.
...
│  │              │   │              │   │              │   │              │ │
│  │  URDF/Xacro  │──▶│   Gazebo     │──▶│  Real Robot  │──▶│  Production  │ │
│  │  CAD Models  │   │   Isaac Sim  │   │  (Limited)   │   │   Fleet      │ │
│  │  Specs       │   │   MuJoCo     │   │              │   │              │ │
│  └──────────────┘   └──────────────┘   └──────────────┘   └──────────────┘ │
│                                                                             │
│        10%              80%              9%               1%                │
│    of dev time      of dev time      of dev time      of dev time          │
│                                                                             │
│  Key Insight: 80% of robotics development happens in simulation!           │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
## What Is Gazebo Ionic?

### Definition

**Gazebo Ionic** is an open-source, physics-based robotics simulator that creates accurate digital twins of robots and environments. It computes:
...
:::note For Beginners
Think of Gazebo like a "video game engine for robots." Just as Unity or Unreal creates virtual worlds for games, Gazebo creates virtual worlds for robots—but with *physically accurate* behavior.
:::

| Dimension | Gazebo Ionic | Isaac Sim | MuJoCo | PyBullet |
|-----------|-----------------|-----------|--------|----------|
| **Best For** | ROS 2 integration | NVIDIA GPU training | Research/RL | Quick prototyping |
| **Physics Quality** | ⭐⭐⭐⭐ Good | ⭐⭐⭐⭐⭐ Excellent | ⭐⭐⭐⭐⭐ Excellent | ⭐⭐⭐ Adequate |
| **ROS 2 Support** | ⭐⭐⭐⭐⭐ Native | ⭐⭐⭐⭐ Via bridge | ⭐⭐ Limited | ⭐⭐ Limited |
| **Learning Curve** | ⭐⭐⭐⭐ Easy | ⭐⭐⭐ Medium | ⭐⭐⭐ Medium | ⭐⭐⭐⭐⭐ Very Easy |
| **GPU Acceleration** | ⭐⭐⭐ Optional | ⭐⭐⭐⭐⭐ Required | ⭐⭐⭐⭐ Good | ⭐⭐ Limited |
| **Multi-Robot** | ⭐⭐⭐⭐ Good | ⭐⭐⭐⭐⭐ Excellent | ⭐⭐⭐ Adequate | ⭐⭐ Limited |
...
**Lesson learned**: "If it doesn't work in simulation, don't bother trying on hardware." — Amazon Robotics engineer
:::
...
- Single robot testing: RTX 5080/6080 sufficient
- 10-robot parallel training: 2x RTX 6090 or H100
- 100+ robot fleet simulation: Multi-GPU server/DGX or cloud (AWS RoboMaker)

**Cost-performance sweet spot**: 2x RTX 6090 server (~$18K) can simulate 100+ robots at 15x real-time using modern GPU-accelerated solvers.
:::
...
│              └─────────────────────────┘                                │
│                                                                         │
│   🎯 Key Insight: 80% of capstone development happens in simulation    │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```

**Latency Trap Warning**: Train/validate in Gazebo (workstation RTX 5080/6080) → Flash weights to Jetson (no cloud dependency).

---

## Implementation

### Step 1: Install Gazebo Ionic

**Installation Script**: `~/install-gazebo-ionic.sh`

```bash
#!/bin/bash
# Install Gazebo Ionic on Ubuntu 24.04
# Requires ROS 2 Kilted Kaiju

set -e

echo "===== Gazebo Ionic Installation ====="

# Add Gazebo repository
sudo wget https://packages.osrfoundation.org/gazebo.gpg -O /usr/share/keyrings/pkg-gazebo-archive-keyring.gpg
echo "deb [arch=$(dpkg --print-architecture) signed-by=/usr/share/keyrings/pkg-gazebo-archive-keyring.gpg] http://packages.osrfoundation.org/gazebo/ubuntu-stable $(lsb_release -cs) main" | sudo tee /etc/apt/sources.list.d/gazebo-stable.list > /dev/null
...
# Verify installation
gz sim --version
# Expected: Gazebo Sim, version 9.x.x

echo "===== Gazebo Ionic Installed Successfully ====="
```

**Execute Installation**:
```bash
chmod +x install-gazebo-ionic.sh
./install-gazebo-ionic.sh
```
...
**ros_gz_bridge** translates Gazebo topics ↔ ROS 2 topics:
...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│   GAZEBO SIMULATION                    ROS 2 ECOSYSTEM                  │
│   ┌─────────────────┐                  ┌─────────────────┐              │
│   │ /gz/camera      │ ───[Bridge]───▶  │ /camera/image   │              │
│   │ /gz/imu         │ ───[Bridge]───▶  │ /imu/data       │              │
│   │ /gz/joints      │ ───[Bridge]───▶  │ /joint_states   │              │
│   │ /gz/cmd_vel     │ ◀──[Bridge]───── │ /cmd_vel        │              │
│   └─────────────────┘                  └─────────────────┘              │
│                                                                         │
...
**SDF Conversion** (URDF → SDF for Gazebo):

Gazebo Ionic uses **SDF (Simulation Description Format)**, but can spawn URDFs directly.
...
```
┌─────────────────────────────────────────────────────────────────────────┐
│                    GAZEBO IONIC ARCHITECTURE                          │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  Gazebo Simulation                                                      │
│  ├── Physics Server (1000 Hz)                                           │
│  │   ├── ODE/Bullet/DART engine                                         │
│  │   ├── Collision detection                                            │
│  │   └── Constraint solver                                              │
│  ├── Rendering Server (30-60 Hz)                                        │
│  │   ├── Ogre 2.3 renderer                                              │
│  │   └── GPU ray tracing (sensors)                                      │
│  └── Sensor Manager                                                     │
│      ├── Camera plugins                                                 │
│      ├── IMU plugins                                                    │
│      └── Depth/Lidar plugins                                            │
...
1. **Headless mode** (no GUI): `gz sim -s world.sdf` — 2-5x faster
2. **Disable shadows**: Add `<cast_shadows>false</cast_shadows>` — 30% faster
3. **Reduce sensor resolution**: 320x240 vs 640x480 — 4x less compute
4. **Increase physics step size**: 2ms vs 1ms — 2x faster (less accurate)

**Benchmark on RTX 5080/6080**:
...
| **Gazebo Ionic** | Open-source simulator with native ROS 2 support |
...
```
Gazebo won't start?
├── Error message about OpenGL
│   ├── Yes → Run with LIBGL_ALWAYS_SOFTWARE=1
│   └── No → Check nvidia-smi for GPU status
└── No error but freezes
    ├── During "Loading world" → Check world file path
    └── After loading → Reduce world complexity

Robot doesn't appear?
├── Launch file errors
│   ├── Yes → Check xacro paths
│   └── No → Check gz topic list for spawn status
└── Robot spawns but falls through floor
    └── Add ground plane to world file
```
...
- [Gazebo Ionic Docs](https://gazebosim.org/docs/ionic)
- [ros_gz_bridge Tutorial](https://gazebosim.org/docs/ionic/ros2_integration)
