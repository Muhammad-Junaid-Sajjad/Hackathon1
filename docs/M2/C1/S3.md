...
- **ROS 2 Humble workspace**: Sourced and functional
...
Waymo's simulation recreates entire cities with millions of objects—buildings, vehicles, pedestrians, traffic signals. They use photorealistic rendering with ray-tracing for camera sensor validation.
...
Spot and Atlas robots train in procedurally generated terrain—stairs, rubble, slopes, narrow passages. They randomize surface friction, step heights, and gap widths to build robust locomotion policies.

**Key metrics they care about:**
- **Terrain difficulty**: 10 levels from flat to extreme
- **Surface variation**: Friction 0.2-1.5, compliance 0.1-10
- **Gap/step randomization**: ±20% of nominal values
...
- Ubuntu 22.04 LTS
- NVIDIA RTX 4070 Ti (12GB VRAM) for real-time rendering
...
1. **Training Environments**: Kitchen, warehouse, office—each with objects, furniture, lighting variations
...
<sdf version="1.9">
  <world name="indoor_lab">
    ...
    <atmosphere type="adiabatic">
      <temperature>298.15</temperature>  <!-- 25°C -->
...
<light type="spot" name="task_light">
  ...
  <spot>
    <inner_angle>0.6</inner_angle>  <!-- ~34° cone -->
    <outer_angle>1.0</outer_angle>  <!-- ~57° falloff -->
    <falloff>1.0</falloff>
  </spot>
...
The trend is toward "infinite worlds"—using generative models to create photorealistic, physically plausible environments at scale.
:::
...
- Randomizes cup positions on counter (±10cm noise)
...
**Constraints:**
- RTF must be ≥ 2.0 for all configurations
...
```
World not loading?
├── Check SDF syntax: gz sdf --check world.world
│   ├── Syntax error → Fix XML (check closing tags)
│   └── No errors → Check file path
└── File path correct?
    ├── Yes → Check Gazebo version compatibility
    └── No → Use absolute path or verify working directory

Objects not visible?
├── Check if spawned: gz model --list
│   ├── Not listed → Spawn command failed
│   └── Listed → Check pose (might be underground or out of view)
├── Check lighting: Are there any lights in world?
└── Check materials: Ensure ambient/diffuse not (0,0,0,0)
```
